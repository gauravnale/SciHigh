...
function ID_from_username($uname){
 ...
 function find_role($uname){
	 return(mysql_result(mysql_query("SELECT `role` FROM `USER` WHERE `username`='$uname'"),0,'role'));
	 //if(mysql_result(mysql_query("SELECT `role` FROM `USER` WHERE `username`='$uname'"),0,'role')=='tutor')
	     //return '2';
	// if(mysql_result(mysql_query("SELECT `role` FROM `USER` WHERE `username`='$uname'"),0,'role')=='counselor')
		// return '3';
 }
?>